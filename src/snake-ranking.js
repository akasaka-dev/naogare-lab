// ---------------------------------------------------------------------------
// snake (LUMEN SNAKE - CYBER) score ranking — isolated in this file so it
// never touches the other games' routes/tables in src/index.js. Reuses the
// shared `game_sessions` and `rate_limits` tables in env.DB (same pattern as
// fuwao's anti-cheat token + rate limiting), but has its own dedicated
// `snake_scores` table.
// ---------------------------------------------------------------------------
...
const GAME_ID = 'snake';
const STAGE = 1; // game_sessions.stage has no meaning for snake — fixed constant so the column stays NOT NULL
const MIN_SCORE = 0;
...
async function handleRanking(env) {
  const { results } = await env.DB.prepare(
    'SELECT name, score, created_at FROM snake_scores ORDER BY score DESC, created_at ASC LIMIT ?1'
...
async function handleScoreSubmit(request, env) {
  ...
  await env.DB.prepare(
    'INSERT INTO snake_scores (name, score, created_at) VALUES (?1, ?2, ?3)'
  ).bind(sanitizeName(name), score, createdAt).run();

  return jsonResponse({ ok: true });
}

export async function routeSnake(request, env, path) {
  if (path === '/api/snake/ranking') {
  ...
  if (path === '/api/snake/session') {
    if (request.method !== 'POST') return errorResponse('method_not_allowed', 405);
    if (!(await checkRateLimit(env, 'snake-session', request, 20, 600))) {
  ...
  if (path === '/api/snake/score') {
    if (request.method !== 'POST') return errorResponse('method_not_allowed', 405);
    if (!(await checkRateLimit(env, 'snake-score', request, 20, 600))) {
      return errorResponse('rate_limited', 429);
    }
    try {
